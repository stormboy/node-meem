...
var Meem = function(def, content) {
	...
	// put settings and values in Meem 
	this.setContent(content || def.content);
...
Meem.prototype._createFacets = function(facetDefs) {
	...
	var handlePropertyRequest = function(request) {
		request.respond(self.properties);
	};
	def = {
		name: "propertiesOut",
		type: "org.meemplex.Properties", 
...
Meem.prototype.setPropertyValue = function(name, value) {
	...
	if (typeof property != 'undefined') {
		...
		if (this.emit) {
			this.emit("property", name, value, oldValue);			// emit event to signal property change
		}
...
Meem.prototype.setContent = function(content) {
	if (typeof content != 'undefined' && content) {
		...
		for (var name in content) {
			this.setPropertyValue(name, content[name]);
		}
		if (this.emit) {
