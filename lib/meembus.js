...
var MeemBus = function(options) {
	...
	console.log("mqtt host: " + mqttOptions.host);
...
MeemBus.prototype.onMessage = function(topic, handler) {
	if (TRACE && DETAIL) {
		console.log("MeemBus: subscribing to: " + topic);
	}
...
MeemBus.prototype._addSubscription = function(topic) {
	...
	if (TRACE && DETAIL) {
		console.log("MeemBus: adding subscription: " + topic + " connected: " + this._connected);
	};
	if (this._connected) {
...
MeemBus.prototype._removeSubscription = function(topic) {
	if (this._subscriptions[topic]) {
		this._subscriptions[topic]--;
		if (this._subscriptions[topic] == 0 && this._mqttClient._connected) {
...
MeemBus.prototype._subscribe = function() {
	for (topic in this._subscriptions) {
		if (TRACE && DETAIL) {
			console.log("MeemBus: subscribing to topic: " + topic);
		}
		this._mqttClient.subscribe(topic);
	}
};
...
MeemBus.prototype._init = function(mqttClient) {
	...
	if (TRACE) {
		console.log("MeemBus: initialise MQTT connection");
	}
	...
	mqttClient.on('connect', function() {
		if (TRACE) {
			console.log('MeemBus: MQTT connected');
		}
	...
	mqttClient.on('close', function() {
		if (TRACE) {
			console.log('MeemBus: MQTT closed');
		}
		self.emit("offline");
	});
	...
	mqttClient.on('error', function(e) {
		if (TRACE) {
			console.log('MeemBus: MQTT error: ' + e);
		}
	});
	...
	mqttClient.addListener('message', function(topic, payload) {
		...
		if (TRACE && DETAIL) {
			console.log('MeemBus: received ' + topic + ' : ' + payload);
		}
...
MeemBus.prototype._handleMessage = function(topic, message) {
	if (TRACE && DETAIL) {
		console.log("MeemBus: emitting : " + topic);
	}
