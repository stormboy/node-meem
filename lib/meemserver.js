...
MeemServer.prototype.addMeem = function(meemSpec, cb) {
	if (this.meems[meemSpec.id]) {
		if (typeof cb !== "undefined") {
			cb(new Error("meem already created: " + meemSpec.id), this.meems[meemSpec.id]);
		}
		return;
	}
	if (TRACE && DETAIL) {
	...
	if (meem.isSubsystem) {
		...
		meem.on("destroyMeem", function(meemId, callback) {
			self.destroyMeem(meemId, callback);
		});
	}
...
MeemServer.prototype.destroyMeem = function(meemId, cb) {
	...
	self.meemStore.removeMeem(meemId, cb);		// remove from persistent store
};
...
MeemServer.prototype.locateMeem = function(meemId, callback) {
	...
	if (!meem) {
		// try to find persisted meem definition to load
		this.meemStore.getMeem(meemId, function(err, def) {
			...
			// try checking meem cache again because we may now have the meem.
			meem = self.meems[meemId];		
			if (meem) {
				callback(null, meem);
			}
			else {
